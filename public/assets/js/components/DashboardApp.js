...
class DashboardApp {
  ...
  async fetchCarData() {
    ...
    const url = `http://${SERVER_HOST}:${SERVER_PORT}/api/cars`;
  ...
  async init() {
    ...
    for (const element of this.filterInput) {
      element.parentNode.addEventListener("click", () => {
        ...
        const selectedRadioNum = document
          .querySelector('input[name="car-size"]:checked')
          .id.replace("car-size-radio", "");
        this.filterCarBy = element.value;
        if (selectedRadioNum !== prevSelectedRadioNum) {
          this.filterInput[
            prevSelectedRadioNum - 1
